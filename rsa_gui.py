...
from rsa_all import encrypt, decrypt

class RSAApp:
    def __init__(self, root):
        self.root = root
        self.root.title("RSA Encryption and Decryption App")
    ...
    def encrypt(self):
        ...
        if text:
            result = encrypt(text.encode(), key.encode()).hex()
        elif file_path:
            with open(file_path, 'rb') as file:
                binary_data = file.read()
            result = encrypt(binary_data, key.encode()).hex()
        else:
    ...
    def decrypt(self):
        ...
        if text:
            result = decrypt(bytes.fromhex(text), key.encode()).decode()
        elif file_path:
            with open(file_path, 'rb') as file:
                ciphertext = file.read()
            result = decrypt(ciphertext, key.encode()).decode()
        else:
            result = "Please enter text or select a file"
